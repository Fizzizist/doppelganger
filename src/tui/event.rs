...
async fn compute_fingerprint(db_path: &str, app: &App) -> Result<u64> {
...
async fn issue_list_fingerprint(conn: &turso::Connection) -> Result<u64> {
    let mut rows = conn
